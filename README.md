...
```
In spark environment, we load the files saved in HDFS. You might ask why we want to execute our data analysis in Spark environment instead of local environment. In fact, we can retrieve files from HDFS to our local drive by `-copyToLocal` in your linux terminal. But then what do we do afterwards? If we want to analyze the data, our terminal does not have any efficient query language. So by retrieving HDFS file in Spark environment, you can easily transform data into SQL table, analyze them, extract, and save. That's why we need Spark environment in the first place. We can install Spark as standalone program (See more in depth in <a href=https://github.com/kckenneth/Spark/blob/master/setup_spark.md>w251 Spark</a> and execute or we just run Spark from the Docker container. 
```
